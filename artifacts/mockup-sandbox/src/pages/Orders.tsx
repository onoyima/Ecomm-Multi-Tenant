...
import { MOCK_ORDERS, formatPrice, type Order } from "@/data/mockData";
...
export function Orders({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  const tabs = ["All", ...new Set(MOCK_ORDERS.map((o) => o.status))];
          ...
          <TabsContent key={tab} value={tab} className="space-y-4 mt-6">
            {filtered.filter((o) => tab === "All" || o.status === tab).length === 0 ? (
              ...
              filtered.filter((o) => tab === "All" || o.status === tab).map((order) => {
                ...
                return (
                  <Card key={order.id} className="border-border/50 hover:border-primary/20 transition-colors cursor-pointer">
